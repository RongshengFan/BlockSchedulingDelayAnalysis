...
def dump(workdir: str, name: str = "original", suffix: str = ".bin") -> str:
    ...
    # finally try cuobjdump -elf to dump elf content and check .nv_debug_ptx_txt
        ...
        # if still not found, try an external cache before giving up
        print("[objdump] PTX not found in CUBIN, checking cache", file=log)
        cache_dir = os.environ.get("NEUTRINO_PTX_CACHE")
        if cache_dir:
            # kernel name may be passed via env for cache lookup
            kern = os.environ.get("NEUTRINO_CURRENT_KERNEL", name)
            cache_path = os.path.join(cache_dir, kern + ".ptx")
            if os.path.exists(cache_path):
                print(f"[cache] loaded PTX from {cache_path}", file=log)
                with open(cache_path, "r") as f:
                    data = f.read()
                shutil.copyfile(cache_path, os.path.join(workdir, name) + ".ptx")
                return data
        # still nothing; return empty string so caller can skip
        print("[objdump] no PTX available", file=log)
        open(os.path.join(workdir, name) + ".ptx", "w").close()
        return ""
...
if __name__ == "__main__":
    ...
    try:
        ...
        os.environ["NEUTRINO_CURRENT_KERNEL"] = kernel_name
        ptx = dump(workdir)
