...
def parse(path: str):
  with open(path, "rb") as f:
    # header consists of eight 4-byte ints (32 bytes)
    hvals = struct.unpack("iiiiiiii", f.read(32))
    header = TraceHeader(*hvals)  # expand tuple into individual args
    sections: List[TraceSection] = []
    for _ in range(header.numProbes):
      # each section entry is two u32 and one u64 (16 bytes)
      size, warpDiv, offset = struct.unpack("IIQ", f.read(16))
      sections.append(TraceSection(size, warpDiv, offset))
    ...
    for i in range(gridSize):
      ...
      for j in range(blockSize // sections[0].warpDiv):
        # each entry is two u64/u32 pairs as defined by probe
        start, elapsed, cuid = struct.unpack("qII", f.read(16))
        records[i].append(block_sched(start, elapsed, cuid))
...
for cur in records:
  ...
  for block in cu_timelines[cur[0].cuid]:
    if block.start+block.elapsed<=cur[0].start:
      sched_times[cur[0].cuid]+=cur[0].start - (block.start + block.elapsed)
